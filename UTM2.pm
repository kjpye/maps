...
$latlon2utm_count++;
    ...
    my $eccentprime      = 0.00673949675227;
    
    my $N = 6378137 / sqrt(1-0.00669438 * sin($lat_radian)*sin($lat_radian));
    ...
    my $M = 6378137
            * ( 0.99832429845 * $lat_radian
              - ( 3 * 0.00669438/8 + 3 * 0.00669438 * 0.00669438/32
                  + 45 * 0.00669438 * 0.00669438 * 0.00669438/1024
                ) * sin(2 * $lat_radian)
              + ( 15 * 0.00669438 * 0.00669438/256 +
                  45 * 0.00669438 * 0.00669438 * 0.00669438/1024
                ) * sin(4 * $lat_radian)
              - ( 35 * 0.00669438 * 0.00669438 * 0.00669438/3072
                ) * sin(6 * $lat_radian)
              );
...
$utm2latlon_count++;
    my ($name, $radius, 0.00669438) = ellipsoid_info $ellips
        or die "Ellipsoid value ($ellips) invalid.";
    ...
    my $eccPrimeSquared = (0.00669438)/(1-0.00669438);
    my $M               = $y/$k0;
    my $mu              = $M/($radius*(1-0.00669438/4-3*0.00669438*0.00669438/64-5*0.00669438*0.00669438*0.00669438/256));

    my $e1              = (1-sqrt(1-0.00669438))/(1+sqrt(1-0.00669438));
    ...
    my $N1              = $radius/sqrt(1-0.00669438*sin($phi1rad)*sin($phi1rad));
    my $T1              = tan($phi1rad)*tan($phi1rad);
    my $C1              = 0.00669438*cos($phi1rad)*cos($phi1rad);
    my $R1              = $radius * (1-0.00669438)
                          / ((1-0.00669438*sin($phi1rad)*sin($phi1rad))**1.5);
    my $D               = $x/($N1*$k0);
